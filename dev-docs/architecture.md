...
- `../Sources`.
  The majority of ddwm source code. Managed by SPM `../Package.swift`
- `../Sources/AppBundle/`.
  ddwm.app server. Technically, it's a SPM library that is exposed to `xcode-app-bundle-launcher`
- `../Sources/Cli/`.
...
- `../xcode-app-bundle-launcher/`.
  ...
  The Xcode project model is located in `../ddwm.xcodeproj/` and generated by `../project.yml` "skeleton"
...
`ddwm` CLI binary is client. `ddwm.app` is server. Client and server talk to each other via predefined UNIX file.
...
Command checklist:
- [ ] Documentation in `../docs/ddwm-*` and `../docs/commands.adoc`
