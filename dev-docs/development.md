...
If you struggle to build ddwm locally, you can also refer to [builds in GitHub Actions](https://github.com/dme86/ddwm/actions?query=branch%3Amain)
...
2.  Install [swiftly](https://github.com/swiftlang/swiftly).
    ...
    Follow installation instructions from the swiftly repository.
3.  If you want to build shell completion, install rust, bash and fish
    -   Install Rust using rustup. https://www.rust-lang.org/tools/install
    -   Install bash and fish using your preferred package manager
4.  If you want to build man pages, install Ruby >= 3.0. I recommend using [rbenv](https://github.com/rbenv/rbenv).
    ...
    -   Install asciidoctor using Ruby `bundler`. `cd ddwm && bundler install`
5.  Install optional `xcbeautify` to make Xcode build logs readable.

## 2. Create codesign certificate

If you want to run ddwm as App Bundle (ddwm.app) you need to create self-signed certificate that will be used to codesign ddwm.
Release artifact is built as App Bundle.
If you only plan to build the debug version of ddwm, you can run it from the terminal and custom certificate is not required.

1.  Open `Keychain Access.app`
2.  Menu -> `Keychain Access` -> `Certificate Assistance` -> `Create a Certificate...`
    -   Name: `ddwm-codesign-certificate`
