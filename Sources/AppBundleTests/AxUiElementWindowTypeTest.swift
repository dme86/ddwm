...
func checkAxDumpsRecursive(_ dir: URL) throws {
    for file in try FileManager.default.contentsOfDirectory(at: dir, includingPropertiesForKeys: nil) {
        ...
        let app = json["Ddwm.AXApp"]!.asDictOrDie
        let appBundleId = (rawJson["Ddwm.App.appBundleId"] as? String).flatMap { KnownBundleId.init(rawValue: $0) }
        let windowLevel = json["Ddwm.windowLevel"].map { MacOsWindowLevel.fromJson($0) ?? dieT() }
        let activationPolicy: NSApplication.ActivationPolicy = .from(string: rawJson["Ddwm.App.nsApp.activationPolicy"] as! String)
        assertEquals(
            json.getWindowType(axApp: app, appBundleId, activationPolicy, windowLevel),
            AxUiElementWindowType(rawValue: rawJson["Ddwm.AxUiElementWindowType"] as? String ?? dieT()),
            additionalMsg: "\(file.path()):0:0: AxUiElementWindowType doesn't match",
        )
        assertEquals(
            json.isDialogHeuristic(appBundleId, windowLevel),
            rawJson["Ddwm.AxUiElementWindowType_isDialogHeuristic"] as? Bool ?? dieT(),
