...
let usage =
    """
    USAGE: \(CommandLine.arguments.first ?? "ddwm") [-h|--help] [-v|--version] <subcommand> [<args>...]
...
struct Main {
    static func main() async {
        ...
        if args.first == "--version" || args.first == "-v" {
            ...
            print(
                """
                ddwm CLI client version: \(cliClientVersionAndHash)
                ddwm.app server version: \(serverVersionAndHash ?? "Unknown. The server is not running")
                """,
            )
            if serverVersionAndHash != nil && cliClientVersionAndHash != serverVersionAndHash {
                eprint(
                    """
                    Warning: ddwm client/server versions don't match. Possible fixes:
                      - Restart ddwm.app (server restart is required after each update)
                      - Reinstall and restart ddwm (corrupted installation)
                    """,
                )
            }
            exit(0)
        ...
        if let e = await connection.startBlocking() {
            exit(stderrMsg: "Can't connect to ddwm server. Is ddwm.app running?\n\(e.localizedDescription)")
        }
            ...
            && hasStdin()
        {
                ...
                parsedArgs is MoveNodeToWorkspaceCmdArgs && (parsedArgs as! MoveNodeToWorkspaceCmdArgs).explicitStdinFlag == nil
            {
                exit(
                    stderrMsg: """
                        ERROR: Implicit stdin is detected (stdin is not TTY). Implicit stdin was forbidden in ddwm v0.20.0.
                        1. Please supply '--stdin' flag to make stdin explicit and preserve old ddwm behavior
                        2. You can also use '--no-stdin' flag to behave as if no stdin was supplied
                        Breaking change issue: https://github.com/dme86/ddwm/issues/1683
                        """,
                )
            }
            var index = 0
        ...
        let windowId = ProcessInfo.processInfo.environment[DDWM_WINDOW_ID].flatMap(UInt32.init)
        let workspace = ProcessInfo.processInfo.environment[DDWM_WORKSPACE]
        ...
        if ans.exitCode != 0 && ans.serverVersionAndHash != cliClientVersionAndHash {
            eprint(
                """
                Warning: ddwm client/server versions don't match
                  - ddwm CLI client version: \(cliClientVersionAndHash)
                  - ddwm.app server version: \(ans.serverVersionAndHash)
                  Possible fixes:
                  - Restart ddwm.app (server restart is required after each update)
                  - Reinstall and restart ddwm (corrupted installation)
                """,
