...
public let socketPath = "/tmp/\(ddwmAppId)-\(unixUserName).sock"
...
public func dieT<T>(
    ...
    let message =
        """
        Please report to:
            https://github.com/dme86/ddwm/discussions/categories/potential-bugs
            Please describe what you did to trigger this error

        Message: \(_message)
        Version: \(ddwmAppVersion)
        Git hash: \(gitHash)
    ...
    if !isUnitTest && isServer {
        showMessageInGui(
            filenameIfConsoleApp: recursionDetectorDuringTermination
                ? "ddwm-runtime-error-recursion.txt"
                : "ddwm-runtime-error.txt",
            title: "ddwm Runtime Error",
            message: message,
        )
    }
