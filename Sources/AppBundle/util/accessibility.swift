...
private func resetAccessibility() {
    _ = try? Process.run(URL(filePath: "/usr/bin/tccutil"), arguments: ["reset", "Accessibility", ddwmAppId])
}
...
let kAXDdwmSynthetic = "Ddwm.synthetic"
        ...
        if let str = a as? String, let commaIndex = str.firstIndex(of: ",") {
            ...
            if let windowId {
                return castToAxUiElementMock([
                    "Ddwm.axWindowId": Json.uint32(windowId),
                    kAXDdwmSynthetic: Json.bool(true),
                ] as AnyObject)
            }
        }
