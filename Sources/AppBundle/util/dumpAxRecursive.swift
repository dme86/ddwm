...
func dumpAxRecursive(_ ax: AXUIElement, _ kind: AxKind, recursionDepth: Int = 0) -> [String: Json] {
    ...
    if !writable.isEmpty { result["Ddwm.AxWritable"] = .string(writable.joined(separator: ", ")) }
    if !failedAxRequest.isEmpty { result["Ddwm.AxFailed"] = .string(failedAxRequest.joined(separator: ", ")) }
    if !ignored.isEmpty { result["Ddwm.AxIgnored"] = .string(ignored.joined(separator: ", ")) }
    return result
}
