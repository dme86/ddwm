...
public func getMessageWindow(messageModel: MessageModel) -> some Scene {
    // Using SwiftUI.Window because another class in ddwm is already called Window
    SwiftUI.Window(messageModel.message?.title ?? ddwmAppName, id: messageWindowId) {
        MessageView(model: messageModel)
            .onAppear {
                // Set activation policy; otherwise, ddwm windows won't be able to receive focus and accept keyboard input
...
public let messageWindowId = "\(ddwmAppName).messageView"
...
public struct Message: Hashable, Equatable {
    ...
    init(type: MessageType = .config, title: String = ddwmAppName, description: String, body: String) {
        self.type = type
        self.title = title
