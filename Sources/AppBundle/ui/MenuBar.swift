...
public func menuBar(viewModel: TrayMenuModel) -> some Scene { // todo should it be converted to "SwiftUI struct"?
    MenuBarExtra {
        let shortIdentification = "\(ddwmAppName) v\(ddwmAppVersion) \(gitShortHash)"
        let identification      = "\(ddwmAppName) v\(ddwmAppVersion) \(gitHash)"
        ...
        Button {
            NSWorkspace.shared.open(URL(string: "https://github.com/sponsors/dme86").orDie())
            viewModel.sponsorshipMessage = sponsorshipPrompts.randomElement().orDie()
        } label: {
            Text("Sponsor ddwm on GitHub")
