...
/// becomes nil, etc.) which tricks ddwm into thinking that all windows were closed.
/// That's why every time a window dies ddwm caches the "entire world" (unless window is already presented in the cache)
/// so that once the screen is unlocked, ddwm could restore windows to where they were
