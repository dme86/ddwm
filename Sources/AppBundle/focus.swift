...
struct LiveFocus: DdwmAny, Equatable {
...
struct FrozenFocus: DdwmAny, Equatable, Sendable {
    let windowId: UInt32?
...
/// But, in general, commands must firstly check --window-id, --workspace, DDWM_WINDOW_ID env and
/// DDWM_WORKSPACE env before accessing the global focus.
...
@MainActor private func onWorkspaceChanged(_ oldWorkspace: String, _ newWorkspace: String) {
    if let exec = config.execOnWorkspaceChange.first {
        ...
        environment["DDWM_FOCUSED_WORKSPACE"] = newWorkspace
        environment["DDWM_PREV_WORKSPACE"] = oldWorkspace
        environment[DDWM_WORKSPACE] = newWorkspace
