...
extension String {
    @MainActor
    func expandFormatVar(obj: DdwmObj) -> Result<Primitive, String> {
        let formatVar = self.toFormatVar()
        switch (obj, formatVar) {
            case (_, .none): break

            case (.window(let w, _), .workspace):
                return w.nodeWorkspace.flatMap(DdwmObj.workspace).map(expandFormatVar) ?? .success(.string("NULL-WOKRSPACE"))
            case (.window(let w, _), .monitor):
                return w.nodeMonitor.flatMap(DdwmObj.monitor).map(expandFormatVar) ?? .success(.string("NULL-MONITOR"))
            case (.window(let w, _), .app):
                return expandFormatVar(obj: .app(w.app))
            case (.window(_, _), .window): break

            case (.workspace(let ws), .monitor):
                return expandFormatVar(obj: DdwmObj.monitor(ws.workspaceMonitor))
...
private func toLayoutString(tc: TilingContainer) -> String {
    switch (tc.layout, tc.orientation) {
        case (.tile, .h): return LayoutCmdArgs.LayoutDescription.h_tiles.rawValue
        case (.tile, .v): return LayoutCmdArgs.LayoutDescription.v_tiles.rawValue
        case (.monocle, _): return LayoutCmdArgs.LayoutDescription.monocle.rawValue
