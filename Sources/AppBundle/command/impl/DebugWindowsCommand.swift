        ...
        switch debugWindowsState {
            ...
            case .notRecording:
                ...
                io.out(
                    ...
                    2. Run 'ddwm debug-windows' once again to finish the session and get the results
                    """,
                )
...
private func dumpWindowDebugInfo(_ window: Window) async throws -> String {
    ...
    result["Ddwm.windowLevel"] = windowLevelJson
    result["Ddwm.axWindowId"] = .uint32(window.windowId)
    result["Ddwm.workspace"] = .stringOrNull(window.nodeWorkspace?.name)
    result["Ddwm.treeNodeParent"] = .string(String(describing: window.parent))
    result["Ddwm.macOS.version"] = .string(ProcessInfo().operatingSystemVersionString) // because built-in apps might behave differently depending on the OS version
    result["Ddwm.App.appBundleId"] = .stringOrNull(window.app.rawAppBundleId)
    result["Ddwm.App.pid"] = .int(Int(window.app.pid))
    result["Ddwm.App.versionShort"] = .stringOrNull(appInfoDic["CFBundleShortVersionString"] as? String)
    result["Ddwm.App.version"] = .stringOrNull(appInfoDic["CFBundleVersion"] as? String)
    result["Ddwm.App.nsApp.activationPolicy"] = .string(window.macApp.nsApp.activationPolicy.prettyDescription)
    result["Ddwm.App.nsApp.execPath"] = .stringOrNull(window.macApp.nsApp.executableURL?.description)
    result["Ddwm.App.nsApp.appBundlePath"] = .stringOrNull(window.macApp.nsApp.bundleURL?.description)
    result["Ddwm.AXApp"] = .dict(try await window.macApp.dumpAppAxInfo())

    let isDialog = try await window.isDialogHeuristic(windowLevel)
    let isWindow = try await window.isWindowHeuristic(windowLevel)
    result["Ddwm.AxUiElementWindowType"] = .string(AxUiElementWindowType.new(isWindow: isWindow, isDialog: { isDialog }).rawValue)
    result["Ddwm.AxUiElementWindowType_isDialogHeuristic"] = .bool(isDialog)
    ...
    result["Ddwm.on-window-detected"] = .array(matchingCallbacks)

    return JSONEncoder.ddwmDefault.encodeToString(result).prettyDescription
