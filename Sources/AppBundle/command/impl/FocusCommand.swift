    ...
    func run(_ env: CmdEnv, _ io: CmdIo) async throws -> Bool {
        ...
        switch args.target {
            case .direction(let direction):
                let windows = target.workspace.tilingClients
                guard let current = target.windowOrNil, let currentIndex = windows.firstIndex(of: current) else {
...
@MainActor private func wrapAroundTheWorkspace(_ target: LiveFocus, _ io: CmdIo, _ direction: CardinalDirection) -> Bool {
    ...
    let windowToFocus = (direction == .left || direction == .up) ? windows.last : windows.first
