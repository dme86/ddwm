    ...
    func run(_ env: CmdEnv, _ io: CmdIo) -> Bool {
           ...
           window.parent is TilingContainer
        {
            ...
            let delta: CGFloat = switch args.units.val {
                case .set(let unit): CGFloat(unit) / 100 - workspace.masterFactor
                case .add(let unit): CGFloat(unit) / 100
                case .subtract(let unit): -CGFloat(unit) / 100
            }
            workspace.masterFactor = min(max(workspace.masterFactor + delta, 0.05), 0.95)
            return true
        }

        let candidates = target.windowOrNil?.parentsWithSelf
            .filter { ($0.parent as? TilingContainer)?.layout.isTileFamily == true }
            ?? []
        ...
        guard let parent else { return io.err("resize command doesn't support floating windows yet https://github.com/dme86/ddwm/issues/9") }
