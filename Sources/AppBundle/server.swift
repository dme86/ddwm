...
func toggleReleaseServerIfDebug(_ state: EnableCmdArgs.State) async {
    ...
    if !isDebug { return }
    let socketFile = "/tmp/\(stableDdwmAppId)-\(unixUserName).sock"
    let connection = NWConnection(to: NWEndpoint.unix(path: socketFile), using: .tcp)
    defer { connection.cancel() }
    if await connection.startBlocking() != nil { // Can't connect, ddwm.app is not running
        return
    }
...
private let serverVersionAndHash = "\(ddwmAppVersion) \(gitHash)"
...
private func newConnection(_ connection: NWConnection) async { // todo add exit codes
    ...
    while true {
        ...
        guard let token: RunSessionGuard = await .isServerEnabled(orIsEnableCommand: command) else {
            await answerToClient(
                exitCode: 1,
                stderr: "\(ddwmAppName) server is disabled and doesn't accept commands. " +
                    "You can use 'ddwm enable on' to enable the server",
            )
            continue
        }
        if let help {
        ...
        if let command {
            ...
            if request.windowId == nil || request.workspace == nil {
                answer.stderr += "\n\nddwm client has sent incomplete JSON request. 'windowId' or/and 'workspace' fields are missing. Please forward your DDWM_WINDOW_ID and DDWM_WORKSPACE environment variables to these JSON fields. If the appropriate environment variables are empty, pass explict 'null' in the JSON."
            }
