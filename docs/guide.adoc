...
:url-search-for-config: https://github.com/search?q=path%3A*ddwm.toml&type=code
...
ddwm tries to find the custom config in two locations:

. `~/.ddwm.toml`
. `+${XDG_CONFIG_HOME}/ddwm/ddwm.toml+`
...
ddwm uses TOML format for the config.
...
If no custom config is found, ddwm will load the default config.
...
[source,shell]
----
cp /Applications/ddwm.app/Contents/Resources/default-config.toml ~/.ddwm.toml
----
...
The initial binding mode that ddwm starts out with is "main".
...
Commands are the thing you use to manipulate ddwm and query its state.

There are two ways on how you can use commands:

. Bind keys to run ddwm commands. Example:
+
[source,toml]
----
[mode.main.binding]
...
* For `dvorak` and `colemak` users, ddwm offers preconfigured presets.
+
[source,toml]
----
[key-mapping]
...
* `exec.inherit-env-vars = true` configures whether inherit environment variables of `ddwm.app` or not. (The default is `true`)
...
That's why unless you override `exec` section in your config, ddwm falls back to the following `exec` configuration:
+
[source,toml]
----
[exec]
...
ddwm stores all windows and containers in a tree.
ddwm tree tiling model is https://i3wm.org/docs/userguide.html#tree[inspired by i3].

*Definition.* Each non-leaf node is called a "Container"

WARNING: i3 has a different terminology.
"container" in i3 is the same as "node" in ddwm.
...
In total, ddwm provides 4 possible layouts:
...
By default, ddwm does two types of tree normalizations:
...
Since Spaces are so hard to deal with, ddwm reimplements Spaces and calls them "Workspaces".
...
When you quit the ddwm or when the ddwm detects that it's about to crash, ddwm will place all windows back to the visible area of the screen.

ddwm shows the name of currently active workspace in its tray icon (top right corner), to give users a visual feedback on what workspace is currently active.

The intended workflow of using ddwm workspaces is to only have one macOS Space (or as many monitors you have, if `Displays have separate Spaces` is enabled) and don’t interact with macOS Spaces anymore.
...
That means, that if ddwm crashes badly you will still be able to manually "unhide" the windows by dragging these few pixels to the center of the screen.
...
Since ddwm needs a free space to hide windows in,
...
For some reason, mission control doesn't like that ddwm puts a lot of windows in the bottom right corner of the screen.
...
* Wrong window may receive focus in multi-monitor setup: https://github.com/dme86/ddwm/issues/101[#101] (Bug in Apple API)
* Wrong borderless Alacritty window may receive focus in *single monitor* setup: https://github.com/dme86/ddwm/issues/247[#247] (Bug in Apple API)
* Performance issues: https://github.com/dme86/ddwm/issues/333[#333]
* macOS randomly switches focus back: https://github.com/dme86/ddwm/issues/289[#289]

When `Displays have separate Spaces` is enabled,
moving windows between monitors causes windows to move between different Spaces which is not correctly handled by the public APIs ddwm uses,
...
|👍 Public Apple API are more stable (which in turn affects ddwm stability)
...
|`if.during-ddwm-startup`
a|
* If `true` then run the callback only during ddwm startup.
* If `false` then run callback only *NOT* during ddwm startup.
...
* `if.during-ddwm-startup = true` is useful if you want to do the initial app arrangement only on startup.

* `if.during-ddwm-startup = false` is useful if you want to relaunch ddwm, but the callback has side effects that you don’t want to run on every relaunch.
...
* You can use xref:commands.adoc#list-apps[`ddwm list-apps`] CLI command to get IDs of running applications
...
exec-on-workspace-change = ['/bin/bash', '-c',
    'sketchybar --trigger ddwm_workspace_change FOCUSED=$DDWM_FOCUSED_WORKSPACE'
]
----

Besides the <<exec-env-vars,`exec.env-vars`>>, the process has access to the following environment variables:

* `DDWM_FOCUSED_WORKSPACE` - the workspace user switched to
* `DDWM_PREV_WORKSPACE` - the workspace user switched from

For a more elaborate example on how to integrate with Sketchybar see
xref:./goodies.adoc#show-ddwm-workspaces-in-sketchybar[]
...
. ddwm takes the assigned monitor of the workspace and makes the workspace visible on the monitor
. ddwm focuses the workspace
...
[NOTE]
====
The only difference between ddwm and i3 is switching to empty workspaces.
When you switch to an empty workspace, ddwm puts the workspace on an assigned monitor; i3 puts the workspace on currently active monitor.

* I find that ddwm model works better with <<observation,_the observation_>> listed above.
* ddwm model is more consistent (it works the same for empty workspaces and non-empty workspaces)
====
...
ddwm uses the API to gently ask windows whether they are dialogs, but ddwm also applies some heuristics.
...
It's fine to hardcode special handling for popular applications, ddwm already does it.
Please see `isDialogHeuristic` function in ddwm sources.
...
If you can't make ddwm handle some keys in your config, please make sure that you don't have keys conflict with other software that might listen to global keys (e.g. skhd, Karabiner-Elements, Raycast)
